...
* `lib/core/`
  * `constants/constants.dart` — Default parameters (e.g. configuration keys).
  * `database/db_helper.dart` — SQLite history database helper.
  * `services/`
    * `auth_service.dart` — Secure OAuth state, token caches.
    * `gmail_service.dart` — Deterministic search query matching configured ID.
